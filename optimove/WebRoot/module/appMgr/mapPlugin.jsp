...
<script type="text/javascript">
var searchUrl = "<%=request.getContextPath()%>/mapmodelmags/getMapModelJson.do?modeltype=3";
var getMapModelTreeUrl = "<%=request.getContextPath()%>/mapmodelmags/getMapModelTree.do?modeltype=3";
var getMapModelUrl = "<%=request.getContextPath()%>/mapmodelmags/getMapmodelmagMap.do?modeltype=3";
	var treeData = "";
	var bdTreeData = "";
	var replacestr = "";
	$(function() {
	...
	function bdCreateHtmlJs() {
		...
		if (replacestr == "") {
			replacestr = "function mapScale() { map.centerAndZoom(new BMap.Point(116.404, 39.915), 11); }";
		}
		$("#bdJs").text(
				"function mapScale() { map.centerAndZoom(new BMap.Point("
						+ bddata + "), 11); }");
		var htmltext = $("#bdHtmlMap").val();
		var result = htmltext.replace(replacestr, $("#bdJs").text());
		replacestr = $("#bdJs").text();
	...
	function bdChangeJs() {
		$("#bdHtmlMap").val(
				$("#bdHtmlMap").val().replace(oldBdJs, $("#bdJs").text()));
		$("#bdHtmlLocation").val(
				$("#bdHtmlLocation").val().replace(oldBdJs, $("#bdJs").text()));
		$("#bdHtmlMapType").val(
				$("#bdHtmlMapType").val().replace(oldBdJs, $("#bdJs").text()));
		$("#bdHtmlMapDefined")
				.val(
						$("#bdHtmlMapDefined").val().replace(oldBdJs,
								$("#bdJs").text()));
		oldBdJs = $("#bdJs").text();
	}
</script>
...
<body class="easyui-layout">
	...
	<%-- <div id="mainPanle" region="center"
		...
		<div id="tabs" class="" fit="true" border="false" cache="false">
			<div title="地图控件库" id='baiduMap'
				style="padding: 1px; overflow: hidden;background-color: white;"
				align="center">
