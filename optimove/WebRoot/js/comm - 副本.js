...
function showTaskUpdate(jsonParam,index) {
	...
	var test = $('#taskDataList').datagrid("getSelections",{index:index,field:jsonParam.title});
	if(test[0].tasktype=='0'){
		...
		setReadonly(jsonParam.readonlyFields);
		initDlg('#addsubjectwindow').dialog({title:jsonParam.title,buttons:[{
			text:'提交',
			iconCls:'icon-ok',
			handler:function(){
       	     //确定按钮点击后的具体处理函数
			...
			setReadonly(jsonParam.readonlyFields);
			initDlg('#addbudgetwindow').dialog({title:jsonParam.title,buttons:[{
			text:'提交',
			iconCls:'icon-ok',
			handler:function(){
            	//确定按钮点击后的具体处理函数
...
function showPowerSetting(jsonParam) {		
	jsonParam=jsonParam||{};
	jsonParam.updateUrl = isEmpty(jsonParam.getSettingDataUrl)?getSettingDataUrl:jsonParam.getSettingDataUrl;
	jsonParam.title = isEmpty(jsonParam.title)?$('#addpowerwindow').attr('title'):jsonParam.title;
	delError();
	var rows = $('#dataList').datagrid("getSelections");
	if(rows.length==0){
		$.messager.alert('提示框','请选择更新数据','warning');  
	...
	onOpen:function(){
			...
			var zTreeNodes=[
						...
						$.fn.zTree.init($("#dept"), setting, zTreeNodes);
						
						$.fn.zTree.init($("#productMM"), setting, zTreeNodes);
						$("#powerTab").tabs({
							width:570,
							height:200
						});
	...
	if(!isEmpty(errors)){
		...
		$(errors).each(function(index,error){
			...
			if(isEmpty(field.lenght)||field.lenght==0){
				...
				field.after("<span id='"+error.field+'Error'+"' style='color:red'>"+error.msg+"</span>");
		});
...
function addOption(constant,select){
	...
	$(constant).each(function(index,row){
		if(!isEmpty(select.attr('default'))&&select.attr('default')==row.value){
			...
			jsonData.push({value:row.value,display:row.display});
		}
	});
...
function ajaxConstants(){
	$.ajax({
		   ...
		   data: {constantKeys:constantKeys},
		   async: false,
		   success: function(data){
				data = convertJson(data);
				if(data.result == 'ok'){
			...
			if(data.result == 'ok'){
				...
				for(var cons in constantDatas[0]){
					if(cons.indexOf('|')>0){
						...
						$(constantDatas).attr(tempKey,constantDatas.attr(cons));
					}
...
//		}else
		...
		$("#"+id).after("<span name='_checkInput' style='color:red'>*</span>");
	});
}
//为空返回false
...
function wait(title){
	...
	$("<div class=\"datagrid-mask\" style=\"z-index: 990000;\"></div>").css({display:"block",width:"100%",height:"100%"}).appendTo("body");
	$("<div class=\"datagrid-mask-msg\" style=\"z-index: 999000;\"></div>").html(title).appendTo("body").css({display:"block",left:(document.body.scrollWidth - 190) / 2,top:(document.body.clientHeight - 45) / 2}); 
}
//删除等待
function closeWait(){
...
function downLoad(jsonData) {
	...
	if(isEmpty(downTitle)&&isEmpty(downfileName)){
		var tabs = $(window.parent.document).find("#tabs").find("li[class='tabs-selected']").find("span")[0];
		downTitle = $(tabs).html();
	...
	if(isEmpty(serviceObjId)&&!isEmpty(methodName)){
		...
		serviceObjId = serviceObjId.substr(0,1).toLowerCase()+serviceObjId.substr(1)+"Mapper";
	}
...
function printHtml(printId,iframeId)
{
	...
	if(isEmpty(iframeId)){
		...
		if(!isEmpty(printId))
			myWindow.document.body.innerHTML=myWindow.document.getElementById(printId).innerHTML;
		myWindow.print();
	}
...
function addDiyDomPower(treeId, treeNode){
	...
	var editStr = "<input type='checkbox' id='diyBtn_" +treeNode.id+"_edit'>编辑<input type='checkbox' id='diyBtn_" +treeNode.id+"_check'>查看";
	aObj.after(editStr);
}
...
function showAddMesswindow(jsonParam){
	...
	delError();
	jsonParam.title=isEmpty(jsonParam.title)?$('#addmessagewindow').attr('title'):jsonParam.title;
	//新增前处理
	if(typeof jsonParam.preHandler  === "function") jsonParam.preHandler();
	
	$('#addmessagewindow').dialog({title:jsonParam.title,buttons:[{
			text:'保存',
			iconCls:'icon-ok',
			handler:function(){
               //确定按钮点击后的具体处理函数
		...
		onOpen:function(){
			...
			var zTreeNodes=[
							...
							"pId":3,
							"name":"子节点4"}];
						$.fn.zTree.init($("#branch"), setting, zTreeNodes);
						$.fn.zTree.init($("#dept"), setting, zTreeNodes);
						$.fn.zTree.init($("#group"), setting, zTreeNodes);
						$("#messageTab").tabs({
							width:550,
							height:200
						});
...
function showProcessor(jsonParam) {	
	...
	initDlg('#processorwindow').dialog({title:jsonParam.title,buttons:[{
		text:'取消',
		iconCls:'icon-cancel',
		handler:function(){
...
function showAddMesswway(jsonParam){
	jsonParam=jsonParam||{};
	jsonParam.updateUrl = isEmpty(jsonParam.setPowerUrl)?setPowerUrl:jsonParam.setPowerUrl;
	jsonParam.title = isEmpty(jsonParam.title)?$('#addpowerwindow').attr('title'):jsonParam.title;
...
function viewReport(jsonp){
	...
	$('<img alt="" id="reportUrl" src="" style="text-align: center;margin-top: 5px;margin-bottom: 5px" >').attr('src',src).appendTo(report);
...
function onbudgetcourseTreeClick(e, treeId, treeNode) {
	var zTree = $.fn.zTree.getZTreeObj("budgetcourseTree"),
...
function showAddDemo(jsonParam){
	...
	$('#addDemo').dialog({title:jsonParam.title,buttons:[{
			text:'新增指标',
			iconCls:'icon-ok',
			handler:function(){
				$('#demoTbl').datagrid('appendRow',{zbselect:"请选择...",name:""});
			}
		},{
			text:'保存',
			iconCls:'icon-ok',
		...
		$('#demoTbl').datagrid({
            ...
            singleSelect:true,
            columns:[[
                      {field:'zbselect',title:'指标',width:265},
                      {field:'name',title:'指标名',width:265}
                      ]],
        onDblClickCell: function(index,field,value){
			var fieldOp=$('#demoTbl').datagrid('options');
    		var col=fieldOp.columns[0];
    		for(var i=0;i<col.length;i++){
    			if(field=='name'&&field==col[i].field){
    				col[i].editor="numberbox";
    			}else if(field=='zbselect'&&field==col[i].field){
    				col[i].editor={type:"combobox",options:{valueField:"name",textField:"name",data:[{productid:"1",name:"11111"},{productid:"2",name:"1221111"},{productid:"3",name:"1331111"}]}};
    			}
    		}
			$(this).datagrid('beginEdit', index);
			var ed = $('#demoTbl').datagrid('getEditor', {index:index,field:field});
			$(ed.target).focus();
		},
		onClickCell:function(index,field,value){
			var rows=$('#demoTbl').datagrid('getRows').length;
...
function showPoint(jsonParam){
	...
	$('#addPoint').dialog({title:jsonParam.title,buttons:[
		 {
			text:'调整',
			iconCls:'icon-ok',
			handler:function(){
					var selected=$('#pointdg').datagrid('getSelected');
					var rowIndex=$('#pointdg').datagrid('getRowIndex',selected);
					var fieldOp=$('#pointdg').datagrid('options');
		    		var col=fieldOp.columns[0];
		    		for(var i=0;i<col.length;i++){
		    			col[i].editor="numberbox";
		    		}
		    		$('#pointdg').datagrid('beginEdit', rowIndex);
					$('#changeNum').dialog({
						title:"调整",
						buttons:[{
							text:'调整',
							...
							handler:function(){
								...
								for(var i=0;i<val.length;i++){
									if(way.val()=="0"){
										if(cent.val()=="0"){
											if(i==0){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'January'});
												selected.January+=selected.January*(parseInt(val[0].value)*0.01);
												$(ed.target).attr('value', selected.January);
											}else if(i==1){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'February'});
												selected.February+=selected.February*(parseInt(val[1].value)*0.01);
												$(ed.target).attr('value', selected.February);
											}else if(i==2){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'March'});
												selected.March+=selected.March*(parseInt(val[2].value)*0.01);
												$(ed.target).attr('value', selected.March);
											}else if(i==3){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'quarter1'});
												selected.quarter1+=selected.quarter1*(parseInt(val[3].value)*0.01);
												$(ed.target).attr('value', selected.quarter1);
											}else if(i==4){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'April'});
												selected.April+=selected.April*(parseInt(val[4].value)*0.01);
												$(ed.target).attr('value', selected.April);
											}else if(i==5){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'May'});
												selected.May+=selected.May*(parseInt(val[5].value)*0.01);
												$(ed.target).attr('value', selected.May);
											}else if(i==6){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'June'});
												selected.June+=selected.June*(parseInt(val[6].value)*0.01);
												$(ed.target).attr('value', selected.June);
											}else if(i==7){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'quarter2'});
												selected.quarter2+=selected.quarter2*(parseInt(val[7].value)*0.01);
												$(ed.target).attr('value', selected.quarter2);
											}else if(i==8){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'July'});
												selected.July+=selected.July*(parseInt(val[8].value)*0.01);
												$(ed.target).attr('value', selected.July);
											}else if(i==9){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'August'});
												selected.August+=selected.August*(parseInt(val[9].value)*0.01);
												$(ed.target).attr('value', selected.August);
											}else if(i==10){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'September'});
												selected.September+=selected.September*(parseInt(val[10].value)*0.01);
												$(ed.target).attr('value', selected.September);
											}else if(i==11){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'quarter3'});
												selected.quarter3+=selected.quarter3*(parseInt(val[11].value)*0.01);
												$(ed.target).attr('value', selected.quarter3);
											}else if(i==12){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'October'});
												selected.October+=selected.October*(parseInt(val[12].value)*0.01);
												$(ed.target).attr('value', selected.October);
											}else if(i==13){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'November'});
												selected.November+=selected.November*(parseInt(val[13].value)*0.01);
												$(ed.target).attr('value', selected.November);
											}else if(i==14){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'December'});
												selected.December+=selected.December*(parseInt(val[14].value)*0.01);
												$(ed.target).attr('value', selected.December);
											}else if(i==15){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'quarter4'});
												selected.quarter4+=selected.quarter4*(parseInt(val[15].value)*0.01);
												$(ed.target).attr('value', selected.quarter4);
											}
										}else{
											if(i==0){
												var ed = $('#pointdg').datagrid('getEditor', {index:rowIndex,field:'January'});
								...
								$('#pointdg').datagrid('endEdit', rowIndex);
								$('#changeNum').dialog('close');
							}
						},
						{
							text:'取消',
		...
		onOpen:function(){
		var users = {total:6,rows:[
		                           {name:"指标1",January:0,February:0,March:0,quarter1:0,April:0,May:0,June:0,quarter2:0,July:0,August:0,September:0,quarter3:0,October:0,November:0,December:0,quarter4:0},
		                           {name:"指标2",January:0,February:0,March:0,quarter1:0,April:0,May:0,June:0,quarter2:0,July:0,August:0,September:0,quarter3:0,October:0,November:0,December:0,quarter4:0},
		                           {name:"指标3",January:0,February:0,March:0,quarter1:0,April:0,May:0,June:0,quarter2:0,July:0,August:0,September:0,quarter3:0,October:0,November:0,December:0,quarter4:0},
		                           {name:"指标4",January:0,February:0,March:0,quarter1:0,April:0,May:0,June:0,quarter2:0,July:0,August:0,September:0,quarter3:0,October:0,November:0,December:0,quarter4:0},
		                           {name:"指标5",January:0,February:0,March:0,quarter1:0,April:0,May:0,June:0,quarter2:0,July:0,August:0,September:0,quarter3:0,October:0,November:0,December:0,quarter4:0},
		                           {name:"指标6",January:0,February:0,March:0,quarter1:0,April:0,May:0,June:0,quarter2:0,July:0,August:0,September:0,quarter3:0,October:0,November:0,December:0,quarter4:0}
	...
	$("#addCent").dialog({
		...
		onOpen:function(){
			$("#centdg").datagrid({
	            ...
	            height:220,
	            singleSelect:true,
	            frozenColumns:[[{field:'kind',title:'分摊类型',width:285}]],
	            columns:[[
	                {field:'cent',title:'分摊比例',width:285}
	            ]],
	            onDblClickCell: function(index,field,value){
					var fieldOp=$(this).datagrid('options');
		    		var col=fieldOp.columns[0];
...
function showProcess(jsonParam){
	...
	$('#addProcess').dialog({title:jsonParam.title,buttons:[{
			text:'保存',
			iconCls:'icon-ok',
			handler:function(){
			...
			$.ajax({
				...
				success: function(result){
					...
					for ( var i = 0; i < branchData.length; i++) {
						...
						var op="<option value='"+branchData[i].branchCode+"'>"+branchData[i].name+"</option>";
						$("#branchData").append(op);
					}
					for ( var j = 0; j < deptData.length; j++) {
						var op="<option value='"+deptData[j].deptCode+"'>"+deptData[j].deptName+"</option>";
						$("#deptData").append(op);
					}
					for ( var k = 0; k < groupData.length; k++) {
						var op="<option value='"+groupData[k].grpId+"'>"+groupData[k].grpCname+"</option>";
						$("#groupData").append(op);
					}
					for ( var l = 0; l < userData.length; l++) {
						var op="<option value='"+userData[l].workId+"'>"+userData[l].userName+"</option>";
						$("#userData").append(op);
					}
				}
				});
...
function processAddPerson(){
	...
	for ( var i = 0; i < selPers.length; i++) {
		...
		if(!oldflag){
			...
			"<td style='height: 20px;font-size: 14px;line-height: -5px;text-align: center;'>"+selPers[i].name+"</td>" +
			...
			"<input name='tempIds' class='oldPer' value='"+selPers[i].id+"' type='hidden'></input>" +
			"<input name='tempBranchIds' class='oldPer' value='"+selPers[i].branchId+"' type='hidden'></input>" +
			"<input name='tempDeptIds' class='oldPer' value='"+selPers[i].deptId+"' type='hidden'></input>" +
			"</td>" +
...
//form数据封装   
    ...
    function showProcessFlow(jsonParam){
    	...
    	$('#addProcessFlow').dialog({title:jsonParam.title,
    		modal:true,
    		onOpen:function(){
    			$("#branchData").empty();
    			$("#deptData").empty();
