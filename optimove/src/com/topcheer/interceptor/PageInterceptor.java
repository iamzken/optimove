...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (page != null) {
			...
			if("oracle".equals(this.getDb_type())){
			...
			}else if("mysql".equals(this.getDb_type())){
				  String countSql = getCountSql(originalSql);
			      Connection connection=mappedStatement.getConfiguration().getEnvironment().getDataSource().getConnection()  ;          
			      PreparedStatement countStmt = connection.prepareStatement(countSql);  
			      BoundSql countBS = copyFromBoundSql(mappedStatement, boundSql, countSql);
			...
			//重新组装分页查询sql语句
			if ("oracle".equals(this.getDb_type())) {// 对原始Sql追加limit
				int offset = (page.getPageNo() - 1) * page.getPageSize()+1;
				int end = page.getPageNo() * page.getPageSize();
				StringBuffer sb = new StringBuffer();
				String sql1 = "select tt2.* from (select tt1.*,rownum rn from ( "+originalSql+" ) tt1 where rownum<="+end+") tt2 where rn>="+offset+" ";
				
				BoundSql newBoundSql = copyFromBoundSql(mappedStatement,boundSql, sql1);
				MappedStatement newMs = copyFromMappedStatement(mappedStatement, new BoundSqlSqlSource(newBoundSql));
				invocation.getArgs()[0] = newMs;
			} else if ("mysql".equals(this.getDb_type())) {// 对原始Sql追加limit
...
//		}
	...
	private String getCountSql(String sql) {
		...
		}else if("oracle".equals(this.getDb_type())){
			result = "select count(1) from ("+sql+") aliasForPage";
		}
		return result;
	}
